...
// GetRandomStr gets random entry from a given string slice
func GetRandomStr(r *rand.Rand, arr []string) string {
	return arr[r.Intn(len(arr))]
}

// GetRandomInt gets random entry from a given int slice
func GetRandomInt(r *rand.Rand, arr []int) int {
	return arr[r.Intn(len(arr))]
}

// GetRandomDateBetween gets random date between given time ranges
func GetRandomDateBetween(r *rand.Rand, from time.Time, to time.Time) time.Time {
	duration := to.Sub(from)
	return from.Add(time.Duration(r.Int63n(int64(duration))))
}

// PersonFirstNames defines various first names of persons, each one is unique
...
// PersonLastNames defines various last names of persons, each one is unique
...
// PersonNameSuffix defines suffixes appeded to the person's full name
var PersonNameSuffix = []string{
	"I", "II", "III",
}

// PersonNamePrefix defines prefixes, prepended to the person's full name
var PersonNamePrefix = []string{
	"Jr", "Sr", "Dr",
}

// CustomNamePart defines a dictionary of words to choose from while forming a random title
var CustomNamePart = []string{
